...
const ICON_PATTERN = /\b(?:tabler|ri|bx|bi|uit|twemoji|line-md)-[a-z0-9-]+\b/g
const SOURCE_EXTENSIONS = new Set(['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.css', '.scss'])
const ALWAYS_INCLUDE_ICONS = ['tabler-lock', 'tabler-lock-open']
const SKIP_DIRECTORIES = new Set(['.next', 'node_modules', '.git'])
const SKIP_FILES = new Set(['generated-icons.css', 'bundle-icons-css.ts'])

async function generateIconsCSS() {
  const __filename = fileURLToPath(import.meta.url)
  const __dirname = dirname(__filename)
  const sourceDir = join(__dirname, '..', '..')
  const usedIcons = await collectUsedIcons(sourceDir)

  if (usedIcons.length === 0) {
    throw new Error('No icon classes were detected in source files')
  }
  ...
  console.log(`Bundled ${usedIcons.length} icon classes from source scan.`)
}
...
async function collectUsedIcons(dir: string): Promise<string[]> {
  const usedIcons = new Set<string>(ALWAYS_INCLUDE_ICONS)

  await collectIconsFromDirectory(dir, usedIcons)

  return Array.from(usedIcons).sort((a, b) => a.localeCompare(b))
}

async function collectIconsFromDirectory(dir: string, usedIcons: Set<string>): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      await collectIconsFromDirectory(fullPath, usedIcons)
      continue
    }

    if (!entry.isFile() || !hasSourceExtension(entry.name) || SKIP_FILES.has(entry.name)) continue

    const content = await fs.readFile(fullPath, 'utf8')
    const matches = content.match(ICON_PATTERN)
